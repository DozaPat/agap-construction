...
import { CheckCircle2, KeyRound, LockOpen, Pencil, Plus, Search, ShieldCheck, UserCog, Users, X } from 'lucide-react';
...
const UsersManagement = () => {
  ...
  const resetPassword = async (managedUser: ManagedUser) => {
    try {
      ...
      setTemporaryPassword(data.temporaryPassword); setNotice(`Temporary password created for ${managedUser.name}.`); await load();
    } catch (requestError) { setError(messageOf(requestError, 'Unable to reset the password.')); }
  };
